...
public class CKFinderTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			out.write("<iframe src=\"" + buildUrl() + "\" width=\"" + this.getWidth() + "\" height=\""
					+ this.getHeight() + "\" " + this.getClassName() + this.getId()
					+ " frameborder=\"0\" scrolling=\"no\"></iframe>");
		} catch (Exception e) {
			try {
				HttpServletResponse resp = (HttpServletResponse) pageContext.getResponse();
				resp.reset();
				resp.sendError(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, "Problem with creating tag");
	...
	private String buildUrl() {
		...
		if (this.disableThumbnailSelection) {
			...
			qs += !isNullOrEmpty(qs) ? "&amp;" : "?";
			qs += "thumbFunc="
					+ (!isNullOrEmpty(this.selectThumbnailFunction) ? this.selectThumbnailFunction
							: this.selectFunction);
			if (!isNullOrEmpty(this.selectThumbnailFunctionData)) {
				try {
					qs += "&amp;tdata=" + URLEncoder.encode(this.selectThumbnailFunctionData, "UTF-8");
				} catch (UnsupportedEncodingException e) {
				...
				try {
					qs += "&amp;tdata=" + URLEncoder.encode(this.selectFunctionData, "UTF-8");
				} catch (UnsupportedEncodingException e) {
		...
		if (!isNullOrEmpty(this.startupPath)) {
			...
			try {
				qs += "start="
						+ URLEncoder.encode(this.startupPath + (this.startupFolderExpanded ? ":1" : ":0"), "UTF-8");
			} catch (UnsupportedEncodingException e) {
	...
	public final void setBasePath(final String basePath) {
		...
		this.basePath = req.getContextPath() + basePath;
	}
	 ...
	 */
	public final void setSelectThumbnailFunctionData(final String selectThumbnailFunctionData) {
	 ...
	 */
	public final void setDisableThumbnailSelection(final boolean disableThumbnailSelection) {
