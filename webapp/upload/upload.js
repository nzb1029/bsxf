    ...
    function addUpload(){
	     ...
	     $("#uploadify").uploadify({
	       ...
	       */  
	        ...
	        buttonText    :"选择文件",
	        'formData'    :{'module': ""+request["module"],'fileName':""+(request["fileName"]?request["fileName"]:""),'fileType':""+(request["fileType"]?request["fileType"]:""),'blockId':""+(request["blockId"]?request["blockId"]:""),'fullpath':""+(request["fullpath"]?request["fullpath"]:"") },//检测FLASH失败调用
			onFallback :function(){
		...
		function loadFiles( ){
			       $.ajax({
			         url:ctx+"/upload/getFileList?module="+request["module"]+"&fullpath="+(request["fullpath"]?request["fullpath"]:""),
			         cache:false,
			         method:"get",
	                 dataType:"json", 
			         success:function(d){
		   ...
		   for(var i=0;i<files.length;i++){
			  ...
			  // if(file.type && file.type!='' && file.type.indexOf(".")!=-1){ 
				      ...
				      //  <a href="#" class="thumbnail">
				       var str= "<a fname='"+file.name+"' class='file_entry'><div class='image'><div class='img' style='background-image: url(\""+path+"\")'></div></div>";
				       if(request["showtype"]=="1")
		     ...
		     $('a.file_entry').contextMenu('myMenu1', 
		          ...
		          bindings: 
		          {
		            'view': function(t,target) {
					    ...
					    { 
					       var path=ctx+"/static-content?contentPath="+request["module"]+"/"+fname+"&fullpath="+(request["fullpath"]?request["fullpath"]:"");
					       $.dialog({
							    id: 'a15',
							    title: fname,
							    lock: true,
		...
		function deleteFile(fname,module){
		      var f=confirm("确定删除吗？");  
		      if(f){
		       $.ajax({
			         ...
			         method:"POST",
	                 dataType:"json", 
	                 data:{fname:fname,module:request["module"],fullpath:(request["fullpath"]?request["fullpath"]:"")},
			         success:function(d){
			          loadFiles();
			         }
	    ...
	    function downFile(fname,module){
	        var path=ctx+"/static-content?download=true&contentPath="+module+"/"+fname+"&fullpath="+(request["fullpath"]?request["fullpath"]:"");
	    ...
	    if(request["adap"]=="true"){//是否需要高度自适应
	        ...
	        window.setInterval(function(){
	              var winHeight = jQuery(window).height();
	   			  ...
	   			  var winh= jQuery("#wrapper").height();
